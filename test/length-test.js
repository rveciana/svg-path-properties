...
tape("Testing the H and V", function(test) {
  var properties = path.pathProperties("m0,0h10");
  test.equal(properties.getLength(), 10, "Length with m and h");

  properties = path.pathProperties("M50,0H40");
  test.equal(properties.getLength(), 10, "Length with m and H");

  properties = path.pathProperties("m0,0v10");
  test.equal(properties.getLength(), 10, "Length with m and v");

  properties = path.pathProperties("M0,50V40");
  test.equal(properties.getLength(), 10, "Length with m and V");
...
tape("Testing z and Z", function(test) {
  var properties = path.pathProperties("m0,0h10z");
  test.equal(properties.getLength(), 20, "Length with z");

  properties = path.pathProperties("m0,0h10Z");
  test.equal(properties.getLength(), 20, "Length with Z");
  test.end();
});
...
tape("Testing Cubic Bézier", function(test) {
  ...
  //C & c
  var properties = path.pathProperties("M100,25C10,90,110,100,150,195");
...
tape("Testing Quadratic Bézier", function(test) {
  ...
  //Q & q
  var properties = path.pathProperties("M200,300 Q400,50 600,300");
  test.inDelta(properties.getLength(), 487.77, 0.1);

  properties = path.pathProperties("M200,300 q200,-250 400,0");
  test.inDelta(properties.getLength(), 487.77, 0.1);

  //T & t
  properties = path.pathProperties("M0,100 Q50,-50 100,100 T200,100");
  test.inDelta(properties.getLength(), 376.84, 0.1);

  properties = path.pathProperties("M0,100 q50,-150 100,0 t100,0");
  test.inDelta(properties.getLength(), 376.84, 0.1);

  properties = path.pathProperties("M0,100 Q50,-50 100,100 T200,100 T300,100");
  test.inDelta(properties.getLength(), 565.26, 0.1);

  //T & t without previous values
  properties = path.pathProperties("M0,100 T200,100");
