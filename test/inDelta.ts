...
export const inDelta = (actual: number[] | number, expected: number[] | number, delta: number) => {
  if (Array.isArray(actual) && Array.isArray(expected)) return inDeltaArray(actual, expected, delta)
